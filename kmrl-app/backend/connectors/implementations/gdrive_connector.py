...
GDRIVE_SCOPES = ['https://www.googleapis.com/auth/gmail.readonly', 'https://www.googleapis.com/auth/drive.readonly', 'https://www.googleapis.com/auth/drive.file', 'https://www.googleapis.com/auth/gmail.modify']
    ...
    def fetch_documents_historical(self, credentials: Dict[str, str], 
        ...
        try:
            ...
            while processed_count < max_historical:
                try:
                    batch_count += 1
                    logger.info(f"Processing Google Drive batch {batch_count}, processed: {processed_count}/{max_historical}")
                    ...
                    logger.info(f"Google Drive batch {batch_count}: Found {len(files)} files, query: {query}")
                    ...
                    for file_info in files:
                        try:
                            # For historical sync, get ALL files regardless of date
                            # Only parse modified_time for consistency
                    ...
                    # Yield remaining documents
                    if current_batch:
                        logger.info(f"Google Drive batch {batch_count} completed: {len(current_batch)} documents")
                        yield current_batch
                        current_batch = []
